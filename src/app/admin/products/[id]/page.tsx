  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (id) {
          const guitar = await adminService.getGuitarById(id);
          setFormData({
            name: guitar.name,
            price: guitar.price,
            quantity: guitar.quantity,
            thumbnail: guitar.thumbnail || '',
            description: guitar.description || '',
            woodType: guitar.woodType || '',
            color: guitar.color || '',
            stringCount: guitar.stringCount || 6,
            category: { id: guitar.category?.id || (cats.length > 0 ? cats[0].id : 0) },
            brand: { id: guitar.brand?.id || (brds.length > 0 ? brds[0].id : 0) }
          });
        }
      } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <h2 className="text-2xl font-bold">Cập Nhật Sản Phẩm #{id}</h2>
      </div>
